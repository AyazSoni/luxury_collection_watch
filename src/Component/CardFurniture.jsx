...
const CardFurniture = ({ product }) => {
  ...
  return (
    <section ref={ref} className="p-10 grid">
      <div className={`w-72 h-[415px] shadow-2xl rounded-3xl p-5 transition-shadow
            ${inView ? 'main shadow-[rgb(167,205,246)_0px_0px_16px]' : 'default shadow-[rgb(207,229,252,_0.95)_0px_0px_16px]'}`}>
        <img 
          className="w-full h-56 rounded-2xl mx-auto transition-opacity duration-300" 
          src={imageSrc} 
          alt={product.name}
          loading="lazy"
          style={{ 
        ...
        <div className="p-3 mt-2 grid gap-0.5">
          ...
          <div className="flex flex-row gap-10">
            ...
            <button onClick={() => { navigate(`/MainProductPage/${product.id}`) }} className="flex justify-center items-center p-2 rounded-full relative bottom-1 left-14 bg-gradient-to-r from-indigo-400 to-cyan-400 shadow-md">
